...
const TestPrakiraan = () => {
  ...
  const [slicePotensi2, setSlicePotensi2] = useState({
    start: 7,
    finish: 15,
    for: 7,
  });
  ...
  const [sliceIndeksKebeningan2, setSliceIndeksKebeningan2] = useState({
    start: 7,
    finish: 15,
    for: 7,
  });
  ...
  const [dataChartGhi2, setDataChartGhi2] = useState([]);
  ...
  const [dataChartPv2, setDataChartPv2] = useState([]);
  ...
  useEffect(() => {
    if (dataGhi.length > 0) {
      ...
      const ghi2 = dataGhi.slice(slicePotensi2.start, slicePotensi2.finish);
