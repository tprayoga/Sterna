        ...
        try {
          ...
          try {
            await axios.post(
              ...
              {
                source: `${imageUrl ? imageUrl.id : user.imgUrl ? user.imgUrl : "null"}`,
              },
              {
                headers: {
            ...
            try {
              const { data } = await axios.post(`${process.env.REACT_APP_URL_API}/edituser`, body, {
                headers: {
                  Accept: "application/json",
                  Authorization: `Bearer ${token}`,
                },
              ...
              const name = data.user.name.includes("(?)") ? data.user.name.replaceAll(" ", "").split("(?)") : data.user.name;
                ...
                setUser({
                  login: {
                    user: {
                      ...
                      imgUrl: data.profil_images[0].source ? parseInt(data.profil_images[0].source) : data.profil_images[0].source,
                    },
            ...
            setLoading(false);
            setLoadingImage(false);
            imageUrl ? failedToast(`Gagal Mengganti Avatar`) : failedToast(`Gagal Update User`);
            console.log(error);
          }
        } catch (error) {
  ...
  return (
    <div className="px-[2%] py-4 min-h-[91vh] 2xl:container mx-auto">
      ...
      <div className="mt-8 grid lg:grid-cols-2 bg-[#EBFFE4] min-h-[75vh] py-4 px-6 shadow border rounded">
          ...
          <div className="flex items-center h-full justify-center flex-col gap-1 mt-8 lg:mt-0 relative">
            <div
              className={`font-medium text-[4rem] z-10 lg:text-[7rem] w-40 h-40 lg:w-72 lg:h-72 rounded-full overflow-hidden flex items-center justify-center duration-150 text-white relative ${
                imageUrl || user?.imgUrl ? "bg-[#F7FFF4] shadow" : "bg-green-500"
            ...
            >
              {imageUrl ? (
                <img src={imageUrl.source} alt="profile" className="w-full h-full object-cover" />
              ) : (
                <div className="flex items-center justify-center w-full h-full">
                  {user?.imgUrl ? (
                    <img src={AvatarProfile.find((avatar) => avatar.id === user?.imgUrl)?.source} alt="profile" className="w-full h-full object-cover" />
                  ) : (
                    <p className="">
                      {" "}
            ...
            <div className={`absolute bg-[#F7FFF4] z-0 shadow rounded duration-700 ${showAvatar ? "h-96 w-16 -right-24" : "h-10 w-16 right-1/2 translate-x-1/2"}`}>
              <div className="h-full relative flex items-center flex-col">
                <div className="h-full overflow-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-thumb-rounded overflow-x-hidden flex flex-col gap-2 py-2">
                  {AvatarProfile.map((ava, idx) => (
                    <div
                      key={idx}
                      className={`cursor-pointer hover:scale-125 duration-150 hover:bg-slate-100 rounded-full ${imageUrl?.id === ava.id || (!imageUrl && user?.imgUrl === ava.id) ? `bg-slate-100 scale-125` : "scale-100"}}`}
                      onClick={() => {
                        setImageUrl(ava);
                      }}
                    >
        ...
        <form className="flex flex-col mt-8 lg:mt-0 justify-between" onSubmit={handleChangeProfile}>
          <div>
            <p className="font-semibold text-3xl lg:text-xl text-center lg:text-left text-slate-800">Informasi Anda</p>

            <div className="mt-6">
              <p className="font-semibold mb-2 text-slate-800">Ubah Nama Pengguna</p>
              <div className=" grid grid-cols-2 gap-2">
                {userInputFormName.map((inp, idx) => (
                  <div key={idx} className={`${idx === 0 || idx === 1 ? "" : "col-span-2"} `}>
                    <UserInput
                      title={inp.title}
                      value={inp.value}
            ...
            <div className="mt-6">
              <label className="flex justify-between items-center">
                <p className="font-semibold mb-2 cursor-pointer hover:opacity-80 duration-150 text-slate-800">Ubah Sandi Pengguna</p>
                <input
                  type="checkbox"
                  className=""
              ...
              <div className=" grid grid-cols-2 gap-2">
                {userInputFormPassword.map((inp, idx) => (
                  <div key={idx} className={`col-span-2 relative`}>
                    <UserInput
                      title={inp.title}
                      classNameInput={`${idx === 2 && !isSamePassword && "outline-red-500 outline hover:outline-red-500/50 duration-150 hover:outline"}`}
                      type="password"
                      value={inp.value}
                      required={inp.required}
                      ...
                      onClickShowPassword={() => {
                        ...
                        newInput[idx].showPassword = !newInput[idx].showPassword;
                    ...
                    {idx === 2 && !isSamePassword && (
                      <div className="absolute -bottom-6">
                        <p className="font-medium text-red-500/80 pt-2 text-xs">*Kata sandi tidak sama</p>
                      </div>
                    )}
                    {passwordLength && (
                      <div className="absolute -top-0.5 right-0">
                        <p className="font-medium text-red-500/80 pt-2 text-xs">*Kata sandi minimal 6 karakter</p>
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="flex justify-end mt-4 lg:mt-0">
            <button className="bg-main-500 text-white font-semibold disabled:opacity-50 px-8 py-1.5 rounded" disabled={loading || loadingImage}>
              {loading || loadingImage ? <AiOutlineLoading3Quarters className="animate-spin w-6 h-6" /> : "Simpan"}
            </button>
          </div>
        </form>
      </div>
...
const UserInput = ({ title = "Title Here", value, type = "text", required, placeholder, classNameInput, classNameTitle, onChange, onClickShowPassword, disabled, showPassword, children, ...props }) => {
  return (
    <label className="relative">
      <p className={`text-slate-500 font-semibold pb-1 ${classNameTitle}`}>{title}</p>
      <input
      ...
      {type === "password" && (
        <div className="cursor-pointer hover:opacity-50 opacity-70 duration-150 absolute right-3 bottom-1" onClick={onClickShowPassword}>
