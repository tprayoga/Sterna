...
const TableAbout = ({ data, column, maxHeight, overflow = true }) => {
  return (
    <div
    ...
    >
      ...
      >
        <table className="relative w-full table-auto shadow-lg">
          <thead>
            <tr className="">
              {column.map((column, columnIndex) => (
                <th
                  key={columnIndex}
                  className={`sticky bg-[#00AF50] font-medium text-left -top-0.5 py-2 text-black px-2 z-10`}
                >
                  {column.name}
                </th>
              ))}
          ...
          {/* <tbody className={`divide-y text-black/80`}> */}
          <tbody className={`text-black`}>
            {data.map((item, dataIndex) => (
              <tr key={dataIndex} className="border-2">
                {column.map((column, columnIndex) => (
          ...
          <thead>
            <tr className="shadow-lg">
              {column.map((column, columnIndex) => (
                <th
                  key={columnIndex}
                  className="sticky font-medium text-left -top-0.5 py-2 text-black bg-[#00AF50] px-2"
                >
                  {column.name}
                </th>
              ))}
